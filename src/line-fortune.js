...
const TAROT = {
  ...
  法王: { brief: '信じる柱が見つかりつつあります', advice: '先に学ぶこと' },
...
/**
 * 短縮版（400〜500字）を生成する。5種のうち姓名判断（姓名あり時のみ）・
...
function buildReportShort({ sei = '', mei = '', birthday, worry = '' } = {}) {
  ...
  const jinkaku = (seimei && seimei.kaku && seimei.kaku.人格) ? seimei.kaku.人格.n : null;
  const shichu = shichuMeishiki(birthday);
  const nikkanKan = shichu.nikkan.kan;
  const nikkanEl = shichu.nikkan.element;
  ...
  if (jinkaku != null) {
    ...
    sections.push({
      head: '姓名判断',
      body: `お名前の中心になる画数は${jinkaku}画。この画数には${nr.nature}の性質があって、あなたらしさがよく出るようです。`,
    });
  }
  ...
  sections.push({
    head: '12星座・西洋占星術',
    body: `${m}月${d}日生まれのあなたの太陽は${sun}。${zod.nature}ことを大切にする、${zod.word}のような人として映ります。今のあなたは${genkyo}の流れの中にいて、繰り返し浮かぶ考えが、答えの形をやわらかく変えているようです。生まれた時刻・場所をいただけると、上昇星座まで読めます。`,
  });
  ...
  sections.push({
    head: '四柱推命',
    body: `あなたの日干は${nikkanKan}（${nikkanEl}）。${gy.nature}の持ち味を持つ日干で、${gy.scene}に力を発揮しやすいようです。今は${gesshi}の気が強く、${gy.rec1}よりも${gy.rec2}を進めるのが、流れに合っているように見えます。`,
  });
